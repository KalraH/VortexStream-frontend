	...
	extraReducers: (builder) => {
		...
		builder.addCase(getAllVideos.fulfilled, (state, action) => {
			...
			if (page === 1) {
				...
				const existingIds = new Set(state.videos.docs?.map(video => video._id) || []);
				const newVideos = action.payload.docs?.filter(video => !existingIds.has(video._id)) || [];

				state.videos = {
					...action.payload, // Keep pagination info (hasNextPage, etc.)
					docs: [...(state.videos.docs || []), ...newVideos] // Only add unique videos
				};
			}
		});
