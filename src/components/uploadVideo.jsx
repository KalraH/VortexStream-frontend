...
import { publishAvideo } from "../store/slices/video.slice";
...
function UploadVideo({ setUploadVideoPopup }) {
	...
	const publishVideo = async (data) => {
		setVideoSize(
			Math.floor(data.videoFile[0].size / (1024 * 1024))
		);
				...
				<div className="relative w-[95vw] sm:w-3/4 h-[80vh] sm:h-[80vh] mx-auto text-white border overflow-y-auto bg-black">
					...
					>
						<section className="h-12 sticky top-0 z-50 border-b w-full bg-[#222222] flex justify-between items-center px-3">
							...
							<div>
								<Button
									className="bg-lime-500 py-1 px-2 font-bold"
						...
						<section className="px-6 py-2">
							<div className="w-full border border-dotted h-44 p-1 flex flex-col gap-3 justify-center items-center text-center">
								...
								<label
									...
									className="cursor-pointer bg-lime-500 text-black font-bold text-sm py-2 px-4"
								>
									Select
									Files
								</label>
